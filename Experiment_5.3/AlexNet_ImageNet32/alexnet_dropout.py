...
class alexnet:
    ...
    def train(self,model):
        ...
        sgd = optimizers.SGD_test(add_ah=0.0, add_noise=0.0, vgg_alex=1.0, a_corr=np.zeros([12,512,512]), h_corr=np.zeros([12,512,512]), lr=learning_rate, decay=lr_decay, momentum=0.9, nesterov=True)
        ...
        historytemp = model.fit(x=x_train,y=y_train,epochs=maxepoches,
                                ...
                                validation_data=(x_test, y_test),callbacks=[reduce_lr,save_weights.save_weights(fname=self.fname)],verbose=2)
...
if __name__ == '__main__':
    ...
    for i in [1]:
        K.clear_session()
        fname = 'rawdata/dropout'
        model = alexnet(fname, x_train, y_train, x_test, y_test)