...
@tf_export(v1=["nn.Noise_Layer"])
@deprecation.deprecated_args(None, "  "
                             " ",
                             " ")
def Noise_Layer(x, keep_prob=None, noise_shape=None, seed=None, name=None,
            rate=None):  # pylint: disable=invalid-name
  try:
    keep = 1. - keep_prob if keep_prob is not None else None
  except TypeError:
  ...
  if rate is None:
    raise ValueError("You must provide a rate to Noise_layer.")

  return Noise_Layer_v2(x, rate, noise_shape=noise_shape, seed=seed, name=name)


@tf_export("nn.Noise_Layer", v1=[])
def Noise_Layer_v2(x, rate, noise_shape=None, seed=None, name=None):  # pylint: disable=invalid-name
  with ops.name_scope(name, "dropout", [x]) as name:
    x = ops.convert_to_tensor(x, name="x")
    if not x.dtype.is_floating:
      raise ValueError("x has to be a floating point tensor since it's going to"
    ...
    rate_gaussian = tf.random.uniform(noise_shape, 1.-2*rate, 1.+2*rate, x.dtype)
    
    def kroc(a,split_data):
        for ii in range(split_data):
            if ii==0:
                temp = [a*a[ii]]
            else:
                temp = tf.concat([temp,[a*a[ii]]],0)
        return temp
    
    if rate_gaussian.shape.ndims==4 and np.random.rand(1)[0]>0.85:
        act_value = tf.reshape(x, [noise_shape[0]*noise_shape[1]*noise_shape[2],-1])
        act_value = tf.reduce_mean(act_value, 0)

        split_data = int(rate_gaussian.shape[3].value/8)
        #zz = tf.zeros([noise_shape[3],noise_shape[3]], x.dtype, name=None)
        for i in range(8):
            z = kroc(act_value[i*split_data:i*split_data+split_data],split_data)
            #z = z / (z.max(axis=1).reshape(-1,1)+0.0001) + np.random.normal(loc=0.0, scale=0.2, size=(split_data,split_data))
            z = z/(tf.reshape(tf.reduce_max(z, reduction_indices=[1]),[-1,1])+0.0001)+tf.random.normal([split_data,split_data], 0.0, 0.2, x.dtype)
            z = tf.matrix_inverse(z)
            z = z * (-tf.ones([split_data,split_data], x.dtype, name=None) + tf.dtypes.cast(2*tf.matrix_diag(np.ones([1,split_data])[0]), x.dtype))  
            z = z / tf.reshape(tf.reduce_sum(tf.abs(z), 1),[-1,1])
            z = z * (tf.ones([split_data,split_data], x.dtype, name=None)-tf.dtypes.cast(tf.matrix_diag(np.ones([1,split_data])[0]), x.dtype))/10+tf.dtypes.cast(tf.matrix_diag(np.ones([1,split_data])), x.dtype)
            if i==0:
                zz = tf.concat([z[0], tf.zeros([split_data,7*split_data], x.dtype, name=None)], 1)
            elif i<7:
                z = tf.concat([tf.zeros([split_data,i*split_data], x.dtype, name=None),z[0]], 1)
                z = tf.concat([z, tf.zeros([split_data,(7-i)*split_data], x.dtype, name=None)], 1)
                zz = tf.concat([zz, z], 0)
            elif i==7:
                z = tf.concat([tf.zeros([split_data,i*split_data], x.dtype, name=None),z[0]], 1)
                zz = tf.concat([zz, z], 0)

        rate_gaussian = tf.matmul(tf.reshape(rate_gaussian,[noise_shape[0]*noise_shape[1]*noise_shape[2],-1]), zz)
        rate_gaussian = tf.reshape(rate_gaussian,[noise_shape[0],noise_shape[1],noise_shape[2],-1])  
